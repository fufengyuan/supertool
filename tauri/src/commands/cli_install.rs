...
/// 目录：~/.supertool、~/.hermes、~/.qclaw、~/.trae-cn、~/.trae、~/.workbuddy、
...
pub fn sync_user_skills(app: tauri::AppHandle) -> Result<Value, String> {
    ...
    // supertool 门户 / Hermes / QClaw / Trae(国内+国际) / WorkBuddy / Reasonix / Claude Code / Codex / zcode 等（~/.agents）
    let targets = [
        home.join(".supertool/skills"),
        home.join(".hermes/skills"),
        home.join(".qclaw/skills"),
