<template>
  <div class="flex flex-col h-full overflow-hidden bg-base-200">
    ...
    <!-- 分组 Tab 条 -->
    <div class="shrink-0 flex items-center gap-1.5 px-4 py-2 border-b border-base-content/10 bg-base-100 overflow-x-auto">
      <button
        class="group-tab"
        :class="{ 'group-tab-active': groupFilter === 'all' }"
        @click="groupFilter = 'all'"
      >
        全部
        <span class="group-tab-count">{{ notes.length }}</span>
      </button>
      <button
        class="group-tab"
        :class="{ 'group-tab-active': groupFilter === '__ungrouped__' }"
        @click="groupFilter = '__ungrouped__'"
      >
        <SvgIcon name="inbox" size="12" /> 未分组
        <span class="group-tab-count">{{ ungroupedCount }}</span>
      </button>

      <template v-for="g in noteGroups" :key="g.id">
        <div v-if="inlineRenameId === g.id" class="flex items-center gap-1 rounded-full bg-base-200 border border-primary/40 pl-2 pr-1 py-0.5">
          <input v-model="inlineRenameName" class="input input-xs w-28 bg-transparent" @keyup.enter="saveRenameGroup(g)" @keyup.esc="inlineRenameId = null" />
        ...
        <button
          v-else
          class="group-tab group/tab"
          :class="{ 'group-tab-active': groupFilter === g.id }"
          @click="groupFilter = g.id"
        >
          <span>{{ g.icon || '📁' }}</span>
          <span class="max-w-[96px] truncate">{{ g.name }}</span>
          <span class="group-tab-count">{{ getGroupNoteCount(g.id) }}</span>
          <span class="flex items-center gap-0.5 opacity-0 group-hover/tab:opacity-100 transition-opacity">
            <span class="p-0.5 rounded hover:bg-base-content/10" title="重命名" @click.stop="startRenameGroup(g)"><SvgIcon name="pencil" size="10" /></span>
            <span class="p-0.5 rounded hover:bg-error/20 hover:text-error" title="删除" @click.stop="confirmDeleteGroup(g)"><SvgIcon name="trash" size="10" /></span>
          </span>
        </button>
      </template>

      <div v-if="creatingGroup" class="flex items-center gap-1 rounded-full bg-base-200 border border-primary/40 pl-2 pr-1 py-0.5">
        <input v-model="createGroupName" ref="createGroupInputRef" class="input input-xs w-28 bg-transparent" placeholder="分组名称" @keyup.enter="saveCreateGroup" @keyup.esc="creatingGroup = false" />
...
.group-tab-active .group-tab-count {
  background: color-mix(in oklab, var(--color-primary-content, #fff) 22%, transparent);
}
/* 选中态 tab 上的管理按钮 hover：用 primary-content 系配色保证对比度 */
.group-tab-active span.p-0\.5:hover {
  background: color-mix(in oklab, var(--color-primary-content, #fff) 18%, transparent);
}
