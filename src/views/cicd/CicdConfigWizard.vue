          ...
          <div v-if="isMultiModule" class="flex flex-col gap-3">
            ...
            <div v-else class="flex flex-col gap-3 rounded-xl border border-primary/20 overflow-hidden">
              ...
              <div class="px-3 pb-3 flex flex-col gap-2.5">
                ...
                <input v-model="draft.parentBuildPath" class="input input-bordered w-full bg-base-200 font-mono" placeholder="留空=代码目录，或填如 ./SRC/mall/seller-api" />
                ...
                <input v-model="draft.outputPath" class="input input-bordered w-full bg-base-200 font-mono" :placeholder="draft.buildTool === 'maven' ? '如 mall-server/target（jar 所在目录；父统一构建必填）' : '如 build/h5、dist；留空自动检测'" />
