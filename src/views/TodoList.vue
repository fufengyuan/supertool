...
const updateTodoTag = async (todo) => {
  try {
    console.log("[updateTodoTag] called");
...
const saveMarkdown = async (id) => {
  try {
    console.log("[saveMarkdown] called");
    const todo = todoStore.todos.find(t => t.id === id)
    if (todo) {
...
const handleSubtaskCompletion = async (data) => {
  if (data.allCompleted) {
    console.log("[handleSubtaskCompletion] called");
...
onMounted(async () => {
    console.log("[components/TodoList.vue] mounted");
    await Promise.all([
      todoStore.loadTodos(),
