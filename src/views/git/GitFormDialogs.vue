<template>
  ...
  >
    <div class="max-w-lg w-full bg-base-100 rounded-xl shadow-2xl p-6" @click.stop>
      <h3 class="text-lg font-semibold m-0 mb-4">保存 Stash</h3>
  ...
  >
    <div class="max-w-lg w-full bg-base-100 rounded-xl shadow-2xl p-6" @click.stop>
      <h3 class="text-lg font-semibold m-0 mb-4">创建 Tag</h3>
      <div class="flex flex-col gap-2.5 mb-4">
  ...
  >
    <div class="max-w-md w-full bg-base-100 rounded-xl shadow-2xl p-6" @click.stop>
      <h3 class="text-lg font-semibold m-0 mb-4">Rebase</h3>
      <div v-if="rebaseInProgress" class="text-center">
        <p class="m-0 mb-4 text-[13px] leading-relaxed">Rebase 进行中，存在冲突。请解决冲突后选择：</p>
        <div class="flex justify-center gap-2">
          <button class="btn btn-ghost btn-sm" @click="$emit('rebase-abort')">中止 Rebase</button>
          <button class="btn btn-primary btn-sm" @click="$emit('rebase-continue')">继续</button>
        </div>
      </div>
      <div v-else>
        <div class="flex flex-col gap-2.5 mb-4">
          <label class="text-xs font-semibold text-base-content/60">Rebase 到</label>
          <select
            :value="rebaseTarget"
            class="p-2 px-2.5 border border-base-content/20 rounded bg-base-200 text-base-content text-[13px] outline-none focus:border-primary"
        ...
        <div class="flex justify-end gap-2">
          ...
          <button class="btn btn-primary btn-sm" @click="$emit('rebase')" :disabled="!rebaseTarget || rebasing">Rebase</button>
        </div>
      </div>
    </div>
  </div>
  ...
  >
    <div class="max-w-sm w-full bg-base-100 rounded-xl shadow-2xl p-6" @click.stop>
      ...
      <div class="flex justify-end gap-2">
        ...
        <button class="btn btn-primary btn-sm" @click="$emit('amend')" :disabled="amending">Amend</button>
      </div>
    </div>
  </div>
</template>
