...
export function useLogsAPI() {
  return {
    ...
    logsLoadMore: async (params: { streamId: string; presetId: string; currentCount: number; batchSize?: number }): Promise<any> => {
...
export interface TauriAPI {
  ...
  fetchPageContentJs: (url: string) => Promise<string>
  // Calculator
...
export function getTauriAPI(): TauriAPI {
  ...
  cachedAPI = {
    ...
    fetchPageContentJs: async (url: string): Promise<string> => tauriCall('fetch_page_content_js', { url }),
    // Subtask
    ...
    // Redis Stream
    dbRedisStreamInfo: async (id: string, dbIndex: number, stream: string) => tauriCall("db_redis_stream_info", { id, dbIndex, stream }),
    dbRedisStreams: async (id: string, dbIndex: number, pattern: string) => tauriCall("db_redis_streams", { id, dbIndex, pattern }),
    dbRedisStreamAdd: async (id: string, dbIndex: number, stream: string, fields: Record<string, unknown>) => tauriCall("db_redis_stream_add", { id, dbIndex, stream, data: fields }),
    dbRedisStreamMessages: async (id: string, dbIndex: number, stream: string, start: string, end: string, count?: number) => tauriCall("db_redis_stream_messages", { id, dbIndex, stream, count, start, end }),
    dbRedisStreamDel: async (id: string, dbIndex: number, stream: string, entryId: string) => tauriCall("db_redis_stream_del", { id, dbIndex, stream, msgId: entryId }),
    dbRedisStreamDelete: async (id: string, dbIndex: number, stream: string) => tauriCall("db_redis_stream_delete", { id, dbIndex, stream }),
    dbRedisStreamConsumers: async (id: string, dbIndex: number, stream: string, group: string) => tauriCall("db_redis_stream_consumers", { id, dbIndex, stream, group }),
    dbRedisStreamPending: async (id: string, dbIndex: number, stream: string, group: string) => tauriCall("db_redis_stream_pending", { id, dbIndex, stream, group }),
    dbRedisStreamGroupCreate: async (id: string, dbIndex: number, stream: string, group: string) => tauriCall("db_redis_stream_group_create", { id, dbIndex, stream, group }),
    dbRedisStreamGroupDestroy: async (id: string, dbIndex: number, stream: string, group: string) => tauriCall("db_redis_stream_group_destroy", { id, dbIndex, stream, group }),
    dbRedisStreamClaim: async (id: string, dbIndex: number, stream: string, group: string, consumer: string, entryId: string) => tauriCall("db_redis_stream_claim", { id, dbIndex, stream, group, consumer, msgIds: [entryId] }),
    dbRedisStreamAck: async (id: string, dbIndex: number, stream: string, group: string, entryId: string) => tauriCall("db_redis_stream_ack", { id, dbIndex, stream, group, msgIds: [entryId] }),
    dbRedisStreamRetry: async (id: string, dbIndex: number, stream: string, group: string, entryId: string) => tauriCall("db_redis_stream_retry", { id, dbIndex, stream, group, consumer: "retry", msgIds: [entryId] }),
    dbRedisStreamTrim: async (id: string, dbIndex: number, stream: string, maxLen: number) => tauriCall("db_redis_stream_trim", { id, dbIndex, stream, count: maxLen }),
    dbRedisZSetRemove: async (id: string, dbIndex: number, key: string, member: string) => tauriCall("db_redis_zset_remove", { id, dbIndex, key, member }),
    dbRedisZSetRange: async (id: string, dbIndex: number, key: string, start: number, stop: number) => tauriCall("db_redis_zset_range", { id, dbIndex, key, start, stop }),
    // Logs
