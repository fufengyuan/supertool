...
export function useWireGuardAPI() {
  return {
    ...
    wireguardConnect: async (configId: string, configName: string, privateKey: string, peerPublicKey: string, peerEndpoint: string, presharedKey?: string): Promise<any> => {
      const res = await tauriInvoke<any>('wireguard_connect', { configId, configName, privateKey, peerPublicKey, peerEndpoint, presharedKey })
      return res.success ? res.data : null
    },
