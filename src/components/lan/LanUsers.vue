...
async function checkNetworkPermission() {
  ...
  try {
    console.log("[checkNetworkPermission] called")
...
async function refreshDiscovery() {
    console.log("[recheckPermission] called")
  scanning.value = true;
  try {
    console.log("[refreshDiscovery] called")
...
async function loadUnreadCounts() {
  try {
    ...
    if (!userId) return
    const counts = await getTauriAPI().getAllUnreadCounts(userId)
...
onMounted(async () => {
    console.log("[components/lan/LanUsers.vue] mounted")
  // 自动启动 LAN 服务
  try {
    const userId = myUserId.value || 'user_' + Date.now().toString(36)
    const userName = myDisplayName.value || 'User'
    await getTauriAPI().startLan(userId, userName)
  } catch (e) {
    console.warn('LAN start failed:', e)
  }
  await loadMyProfile();
  try {
  ...
  cleanupIpcListeners.push(() => clearInterval(refreshTimer));
});
...
async function chooseReceivePath() {
  try {
    console.log("[chooseReceivePath] called")
...
.profile-card:hover {
  ...
  border-color: var(--accent-color, #8839ef);
}
...
.user-status-dot {
  ...
  height: 12px;
  border-radius: 50%;
  border: 2.5px solid var(--card-bg, #1a1a2e);
  transition: all 0.3s ease;
}
