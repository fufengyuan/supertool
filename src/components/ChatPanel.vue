<template>
  <div
    :class="[
      'fixed bottom-5 right-5 w-[720px] h-[650px] bg-base-100 rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.25)] flex flex-col z-[1000] transition-all duration-300 ease border border-base-content/10 overflow-hidden',
      {
  ...
  >
    ...
    <div v-if="isDragOver" class="absolute inset-0 bg-[rgba(102,126,234,0.15)] z-[9999] flex items-center justify-center pointer-events-none border-3 border-dashed border-[rgba(102,126,234,0.6)] rounded-xl">
      <div class="flex flex-col items-center gap-3 text-primary text-lg font-semibold">
        <SvgIcon name="upload" size="48" strokeWidth="1.5" />
        <p>松开发送文件</p>
      </div>
    </div>

    <div class="flex justify-between items-center p-[14px_16px] bg-gradient-to-br from-[#667eea] to-[#764ba2] text-white shrink-0">
      <div class="flex items-center gap-2 min-w-0">
        <h3 class="text-sm font-semibold m-0 whitespace-nowrap overflow-hidden text-ellipsis">与 {{ peer.name }} 的聊天</h3>
        <span v-if="peer.version" class="text-[11px] opacity-70 shrink-0" :title="peer.version === '2.0' ? '✅ 兼容' : '⚠️ 版本较低'">v{{ peer.version }}</span>
      </div>
      <div class="flex items-center gap-1.5">
        ...
        <button class="bg-none border-none text-white text-base cursor-pointer opacity-80 w-7 h-7 flex items-center justify-center rounded transition-all duration-200 hover:opacity-100 hover:bg-white/15" @click="$emit('close')"><SvgIcon name="x" size="14" /></button>
      </div>
    </div>

    <div class="flex-1 p-3 overflow-y-auto flex flex-col gap-1 bg-base-200 scrollbar-thin" ref="messagesContainerRef" @scroll="handleScroll">
      <!-- 加载更多提示 -->
      <div v-if="isLoadingMore" class="text-center p-2 text-base-content/60 text-xs">
        <span class="inline-block w-3 h-3 border-2 border-base-content/60 border-t-transparent rounded-full animate-spin mr-1.5 align-middle"></span> 加载历史消息...
      </div>
      <div v-if="noMoreMessages" class="text-center p-[4px_8px] text-base-content/60 text-[11px] opacity-60">没有更多消息了</div>
