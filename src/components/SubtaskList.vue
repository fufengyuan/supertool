...
import SubtaskItem from './subtask/SubtaskItem.vue';
import SubtaskInput from './subtask/SubtaskInput.vue';
...
const loadSubtasks = async () => {
  if (!getTauriAPI()) return;
  try {
    console.log("[SubtaskList.vue] loadSubtasks() called");
    subtasks.value = await getTauriAPI().getSubtasksForTodo(props.todoId); }
...
const handleAdd = async (text: string) => {
  ...
  if (!getTauriAPI()) return;
  try {
    await getTauriAPI().addSubtask(newSubtask);
...
const toggleSubtask = async (subtask: any) => {
  if (!getTauriAPI()) return;
  try {
    console.log("[SubtaskList.vue] toggleSubtask() called");
    const updated = { ...subtask, completed: !subtask.completed, updatedAt: new Date().toISOString() };
    await getTauriAPI().updateSubtask(updated.id, updated);
...
const saveSubtaskEdit = async (subtask: any) => {
  ...
  if (!text || !getTauriAPI()) {
    console.log("[SubtaskList.vue] saveSubtaskEdit() called");
...
const deleteSubtask = async (subtask: any) => {
  if (!getTauriAPI()) return;
  try {
    console.log("[SubtaskList.vue] deleteSubtask() called");
    await getTauriAPI().deleteSubtask(subtask.id);
...
const updateTodoCompletion = async () => {
  if (!getTauriAPI()) return;
  try {
    console.log("[SubtaskList.vue] updateTodoCompletion() called");
    const allCompleted = await getTauriAPI().updateTodoCompletionBasedOnSubtasks(props.todoId);
