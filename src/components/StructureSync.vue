...
console.log("[StructureSync.vue] component loaded")
...
async function copyAllSql() {
  const allSql = Array.from(selectedSqls.value).join('\n')
...
async function copySingleSql(idx: number) {
  const sqls = Array.from(selectedSqls.value)
  await navigator.clipboard.writeText(sqls[idx])
  toast.success('已复制')
}
...
function getCompactValue(value: any): string {
  ...
  if (typeof value === 'object') {
    ...
    if (value.nullable !== undefined) parts.push(value.nullable ? 'NULL' : 'NOT NULL')
    if (value.default !== undefined && value.default !== null) parts.push(`DEFAULT ${value.default}`)
    if (value.comment) parts.push(`COMMENT '${value.comment}'`)
