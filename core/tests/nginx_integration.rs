...
fn test_production_config() {
    ...
    let prepay = config.servers.iter().find(|s| s.server_name.starts_with('~'));
    assert!(prepay.is_some(), "Regex server_name should exist");
    if let Some(p) = prepay {
        assert_eq!(p.locations.len(), 6, "prepay server should have 6 locations");
        assert_eq!(p.ssl, 1, "prepay should have SSL");
        assert_eq!(p.http2, 1, "prepay should have http2");
    }

    // Verify api-shop server
    let api_shop = config.servers.iter().find(|s| s.server_name == "api-shop.gmceo.cn");
    assert!(api_shop.is_some(), "api-shop.gmceo.cn should exist");
