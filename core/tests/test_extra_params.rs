...
fn test_parser_captures_extra_params() {
    ...
    let topup = parsed
        .servers
        .iter()
        .find(|s| s.server_name.contains("topup-.*"))
        .unwrap();
    assert!(
        !topup.extra_params.is_empty(),
        "topup server should have extra_params"
    );
    assert_eq!(
        topup.extra_params[0].name, "client_max_body_size",
        "first extra param should be client_max_body_size"
    );
    assert!(
        topup.extra_params[0].value.contains("20M"),
    ...
    let topup_srv = parsed
        .servers
        .iter()
        .find(|s| s.server_name.contains("topup-.*"))
        .unwrap();
    let admin_api_loc = topup_srv
        .locations
        .iter()
