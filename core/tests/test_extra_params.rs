...
fn test_parser_captures_extra_params() {
    ...
    let topup = parsed.servers.iter().find(|s| s.server_name.contains("topup-.*")).unwrap();
    assert!(!topup.extra_params.is_empty(), "topup server should have extra_params");
    assert_eq!(topup.extra_params[0].name, "client_max_body_size", "first extra param should be client_max_body_size");
    assert!(topup.extra_params[0].value.contains("20M"), "client_max_body_size should be 20M");

    let api_shop = parsed.servers.iter().find(|s| s.server_name.contains("api-shop")).unwrap();
    assert!(!api_shop.extra_params.is_empty(), "api-shop server should have extra_params");
    assert_eq!(api_shop.extra_params[0].name, "client_max_body_size", "first extra param should be client_max_body_size");
    assert!(api_shop.extra_params[0].value.contains("200M"), "client_max_body_size should be 200M");
    ...
    let unionmch = parsed.servers.iter().find(|s| s.server_name.contains("unionmch")).unwrap();
    let api_loc = unionmch.locations.iter().find(|l| l.path == "/api").unwrap();
    // proxy_set_header, proxy_redirect are all handled by header flag / generator logic
    // So extra_params for /api should be 0
    assert_eq!(api_loc.extra_params.len(), 0,
    ...
    assert!(root_loc.extra_params.len() >= 5, "root location should have >=5 extra params");
    assert!(root_loc.extra_params.iter().any(|e| e.name == "gzip_static" && e.value.contains("on")),
        "should have gzip_static on");
    assert!(root_loc.extra_params.iter().any(|e| e.name == "add_header" && e.value.contains("Cache-Control")),
        "should have add_header Cache-Control");
    assert!(root_loc.extra_params.iter().any(|e| e.name == "gzip_types"),
        "should have gzip_types");

    // CORS headers in topup /admin-api/ — handled by cros flag, not extra_params
    let topup_srv = parsed.servers.iter().find(|s| s.server_name.contains("topup-.*")).unwrap();
    let admin_api_loc = topup_srv.locations.iter().find(|l| l.path == "/admin-api/").unwrap();
    assert!(admin_api_loc.cros, "should have cros=true for CORS location");
    // WebSocket headers in /infra — handled by websocket flag, not extra_params
    let topup_srv = parsed.servers.iter().find(|s| s.server_name.contains("topup-.*")).unwrap();
    let infra_loc = topup_srv.locations.iter().find(|l| l.path == "/infra").unwrap();
    assert!(infra_loc.websocket, "should have websocket=true for WebSocket location");
...
fn test_extra_params_round_trip() {
    ...
    for (srv_idx, srv) in parsed.servers.iter().enumerate() {
        ...
        assert_eq!(srv.extra_params.len(), gs.extra_params.len(),
            "Server {} extra_param count mismatch: orig={} gen={}",
            srv.server_name, srv.extra_params.len(), gs.extra_params.len());
        for (loc_idx, loc) in srv.locations.iter().enumerate() {
            let gl = &gs.locations[loc_idx];
            assert_eq!(loc.extra_params.len(), gl.extra_params.len(),
                "Server {} Location '{}' extra_param count mismatch: orig={} gen={}",
                srv.server_name, loc.path, loc.extra_params.len(), gl.extra_params.len());
        }
    }
    ...
    for (ui, up) in parsed.upstreams.iter().enumerate() {
        ...
        assert_eq!(up.extra_params.len(), gu.extra_params.len(),
            "Upstream {} extra_param count mismatch: orig={} gen={}",
            up.name, up.extra_params.len(), gu.extra_params.len());
    }
    ...
    for check in &text_checks {
        assert!(generated.contains(check), "Generated config missing: {}", check);
    }

    eprintln!("✅ Round-trip: ALL extra params preserved in generated config");
    eprintln!("   Generated {} bytes, {} servers, {} upstreams, {} locations total",
        generated.len(), gen_parsed.servers.len(), gen_parsed.upstreams.len(),
        gen_parsed.servers.iter().map(|s| s.locations.len()).sum::<usize>());
}
