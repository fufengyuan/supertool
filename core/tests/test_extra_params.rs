...
fn test_parser_captures_extra_params() {
    ...
    let prepay = parsed.servers.iter().find(|s| s.server_name.contains("prepay-.*")).unwrap();
    assert!(!prepay.extra_params.is_empty(), "prepay server should have extra_params");
    assert_eq!(prepay.extra_params[0].name, "client_max_body_size", "first extra param should be client_max_body_size");
    assert!(prepay.extra_params[0].value.contains("20M"), "client_max_body_size should be 20M");
    ...
    let prepay_srv = parsed.servers.iter().find(|s| s.server_name.contains("prepay-.*")).unwrap();
    let admin_api_loc = prepay_srv.locations.iter().find(|l| l.path == "/admin-api/").unwrap();
    ...
    let prepay_srv = parsed.servers.iter().find(|s| s.server_name.contains("prepay-.*")).unwrap();
    let infra_loc = prepay_srv.locations.iter().find(|l| l.path == "/infra").unwrap();
