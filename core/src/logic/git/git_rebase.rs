...
pub async fn git_rebase_interactive(repo_path: &str, base_commit: &str, operations: Vec<Value>) -> Result<Value, String> {
    // 1. Create todo file with operations
    let repo_dir = PathBuf::from(repo_path);
    let git_dir = repo_dir.join(".git");
    
    // Create todo file content
    let todo_content = operations.iter()
        .map(|op| {
    ...
    let output = Command::new(&git_bin)
        ...
        .current_dir(repo_path)
        .env("GIT_SEQUENCE_EDITOR", format!("cp {} \"$1\"", temp_todo.display()))
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .output()
        .await
...
pub async fn git_rebase_todo_list(repo_path: &str, base_commit: &str) -> Result<Value, String> {
    ...
    let commits: Vec<Value> = output.lines()
        .filter(|l| !l.is_empty())
        .map(|line| {
