...
impl super::CoreService {
    ...
    pub async fn get_all_lan_messages(&self) -> Result<Value, String> {
        let result = self.with_db(|db| {
            ...
            let chat_msgs =
                crate::db::lan::get_all_chat_messages(db.conn()).map_err(|e| e.to_string())?;
            let transfers =
                crate::db::lan::get_all_file_transfers(db.conn()).map_err(|e| e.to_string())?;
            Ok::<_, String>(json!({
                "messages": msgs,
    ...
    pub async fn insert_lan_user(&self, user: crate::db::lan::LanUser) -> Result<(), String> {
        self.with_db(|db| crate::db::lan::insert_user(db.conn(), &user).map_err(|e| e.to_string()))
    }
    #[allow(dead_code)]
