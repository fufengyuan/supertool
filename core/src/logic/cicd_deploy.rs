...
async fn do_build(
    ...
    if has_modules && config.parent_build_mode && config.build_tool.as_deref() == Some("maven") {
        ...
        if !maven_modules.is_empty() {
            ...
            // 单体模式残留（如 "SRC/mall"），与已指向聚合根的 localPath join 会产生双重前缀。
...
fn resolve_module_dir(project_path: &Path, rel: Option<&str>) -> PathBuf {
    ...
    // 末段回退：SRC/mall/base-api → base-api
...
mod single_deploy_tests {
    ...
    fn single_deploy_root_prefers_parent_build_path() {
        ...
        c.parent_build_path = Some("./SRC/front/mall-h5".into());
        let root = single_deploy_root(&c, Path::new("/proj"));
        assert_eq!(root, PathBuf::from("/proj/SRC/front/mall-h5"));
    ...
    fn module_label_uses_relative_paths_not_project_name() {
        let mut c = base_config();
        c.parent_build_path = Some("SRC/front/mall-h5".into());
        assert_eq!(single_deploy_module_label(&c), Some("SRC/front/mall-h5".into()));
    ...
    fn resolve_module_dir_falls_back_to_last_segment() {
        ...
        // 项目根下只有 base-api（localPath 已是子目录 SRC/mall 的场景）
        fs::create_dir_all(tmp.join("base-api")).unwrap();

        // 存量模块行含仓库前缀：join 双重前缀不存在 → 末段回退命中
        assert_eq!(
            resolve_module_dir(&tmp, Some("SRC/mall/base-api")),
            tmp.join("base-api")
        );
        // 正常相对路径直接命中
        assert_eq!(
    ...
    fn sanitize_cfg(overrides: &[(&str, bool)]) -> crate::db::cicd::CicdConfig {
        ...
        let mut c = CicdConfig {
            ...
            parent_build_mode: true,
            parent_build_path: "SRC/mall".into(),
            ...
            incremental_upload: true,
            health_check_retries: 3,
            output_path: Some("mall-server/target".into()),
    ...
    fn sanitize_non_maven_clears_tool_specific_fields() {
        ...
        assert_eq!(m.build_path.as_deref(), Some("SRC/mall"));
