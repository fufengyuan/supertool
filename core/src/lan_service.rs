...
use crate::db::lan::{self, ChatMessage, FileTransfer as DbFileTransfer};
use crate::lan_emitter::LanEmitter;
...
impl LanService {
    ...
    pub fn start(&self) -> Result<(), String> {
        ...
        // Setup receive path
        let receive_path = crate::logic::data_dir::received_files_dir()
            .to_string_lossy()
            .to_string();
        ...
        *self.local_ip.lock().unwrap() = local_ip.clone();
        ...
        let recv_app_handle = self.emitter.clone();
        ...
        // ===== Heartbeat thread (peer timeout check only, no UDP broadcast) =====
        {
            ...
            let heartbeat_app = self.emitter.clone();
        ...
        // ===== TCP accept thread =====
        {
            ...
            let db_conn = Arc::clone(&self.db_conn);
            let tcp_app_handle = self.emitter.clone();
    ...
    pub fn set_emitter(&self, _new_emitter: Arc<dyn LanEmitter>) {
        // Runtime emitter replacement not supported yet.
        // The NoopEmitter is used by default.
        // Override by constructing with emitter directly in the future.
    }

    fn emit_event(&self, event: &str, payload: &serde_json::Value) {
        self.emitter.emit(event, payload.clone());
    }
        ...
        match msg_type {
            "heartbeat" | "discovery" => {
                ...
                if let Some(ref peer_avatar_ref) = peer_ref.avatar {
                        ...
                        && !peer_avatar_ref.starts_with("avatar:peer_")
                    {
                        ...
                        let data_dir = crate::logic::data_dir::resolve_data_dir();
                ...
                if is_new {
                    ...
                    {  // emitter available directly
                        ...
                        let _ = emitter.emit("lan-peer-discovered", payload);
                    }
                }
            ...
            "message" => {
                if let Ok(msg) = serde_json::from_value::<LanMessage>(data.clone()) {
                    ...
                    {  // emitter available directly
                        let _ = emitter.emit("lan-message-received", data.clone());
                    }
                }
            }
            "avatar_update" => {
            ...
            "file_start" => {
                ...
                ) = (file_id, file_name, file_size, from_id, from_name)
                {
                    ...
                    {  // emitter available directly
                        ...
                        let _ = emitter.emit("lan-file-transfer-started", payload);
                    }
                }
            }
            "file_start_ack" => {
            ...
            | "collaboration_ended" => {
                ...
                {  // emitter available directly
                    ...
                    let _ = emitter.emit(event_name, data.clone());
                }
            }
            _ => {}
        }
    ...
    fn handle_tcp_connection(
        ...
        if header_type == "MESSAGE" {
            ...
            let _ = emitter.emit("lan-message-received", serde_json::json!({
                "type": "message",
                "from": sender_id,
                "fromName": sender_id,
                "to": my_user_id,
            ...
            let _ = notify_rust::Notification::new().summary(&sender_id).body(&content_preview).appname("SuperTool").show();
            return;
        }
        ...
        for peer in peers_map.values_mut() {
            if peer.online && (now - peer.last_seen) > (PEER_TIMEOUT_SECS as i64 * 1000) {
                ...
                {  // emitter available directly
                    ...
                    let _ = emitter.emit("lan-peer-lost", payload);
                }
            }
        }
    }
    ...
    pub fn send_file(
        ...
        let transfers = Arc::clone(&self.file_transfers);
        let log = Arc::clone(&self.log_buffer);
        let send_emitter = self.emitter.clone();
        let db_conn = Arc::clone(&self.db_conn);
    ...
    pub fn save_temp_file(&self, base64_data: &str, file_name: &str) -> Result<String, String> {
        let temp_dir = crate::logic::data_dir::lan_temp_dir();
...
struct NoopEmitter;
impl LanEmitter for NoopEmitter {
