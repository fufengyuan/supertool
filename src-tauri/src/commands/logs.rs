...
fn stream_server_logs(
    ...
    let server = match core.db_read(|conn| {
        conn.query_row(
            ...
            |row| {
                ...
                let decrypted_pw = raw_pw.map(|pw| crate::encryption::try_decrypt_password(&pw));
                Ok(serde_json::json!({
    ...
    let config = crate::core::ssh::SshServerConfig {
...
pub fn write_system_log(level: String, prefix: String, message: String) {
    crate::core::system_logger::SystemLogger::write_frontend_log(&level, &prefix, &message);
}
