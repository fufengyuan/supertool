...
pub async fn connect_mysql(config: &DbConnectionConfig) -> Result<DbConnection, String> {
    let decrypted_pw = config.password.as_deref().map(|pw| crate::encryption::try_decrypt_password(pw));
...
pub async fn connect_postgres(config: &DbConnectionConfig) -> Result<DbConnection, String> {
    let decrypted_pw = config.password.as_deref().map(|pw| crate::encryption::try_decrypt_password(pw)).unwrap_or_default();
    let conn_str = format!(
...
pub async fn db_connect(config: DbConnectionConfig) -> Result<serde_json::Value, String> {
    log::info!("[Tauri CMD] db_connect() called, type={}, host={}", config.db_type, crate::core::log_sanitizer::sanitize_string(&config.host));
...
pub async fn db_test(config: DbConnectionConfig) -> Result<serde_json::Value, String> {
    log::info!("[Tauri CMD] db_test() called, type={}, host={}", config.db_type, crate::core::log_sanitizer::sanitize_string(&config.host));
