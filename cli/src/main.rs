...
async fn main() {
    ...
    match &cli.command {
        ...
        types::Commands::Todo { action } => {
            ...
            if let Err(e) = cmd_todo(&mut rt, action).await {
        ...
        types::Commands::Subtask { action } => {
            ...
            if let Err(e) = cmd_subtask(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        types::Commands::Project { action } => {
            ...
            if let Err(e) = cmd_project(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        types::Commands::Server { action } => {
            ...
            if let Err(e) = cmd_server(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        types::Commands::Cicd { action } => {
            ...
            if let Err(e) = cmd_cicd(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        types::Commands::Db { action } => {
            ...
            if let Err(e) = cmd_db(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        types::Commands::Log { action } => {
            ...
            if let Err(e) = cmd_log(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        types::Commands::Git { action } => {
            ...
            if let Err(e) = cmd_git(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        types::Commands::Mfa { action } => {
            ...
            if let Err(e) = cmd_mfa(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        types::Commands::Note { action } => {
            ...
            if let Err(e) = cmd_note(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        types::Commands::Accounting { action } => {
            ...
            if let Err(e) = cmd_accounting(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
        ...
        types::Commands::Nginx { action } => {
            ...
            if let Err(e) = cmd_nginx(&mut rt, action).await {
                print_error(&e.to_string());
                process::exit(1);
            }
        }
