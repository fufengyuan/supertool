...
pub async fn cmd_note(runtime: &mut CliRuntime, action: &crate::types::NoteCommands) -> Result<()> {
    ...
    match action {
        ...
        NoteCommands::Update {
            ...
            if let Some(t) = title {
                updates.insert("title".to_string(), serde_json::Value::String(t.clone()));
            }
            if let Some(c) = content {
                updates.insert("content".to_string(), serde_json::Value::String(c.clone()));
            }
            if let Some(g) = group_id {
                updates.insert("groupId".to_string(), serde_json::Value::String(g.clone()));
            }
            if let Some(t) = tags {
                updates.insert("tags".to_string(), serde_json::Value::String(t.clone()));
            }
            updates.insert(
                "updatedAt".to_string(),
        ...
        NoteCommands::AddGroup { name, color } => {
            let data =
                serde_json::json!({ "name": name, "color": color.as_deref().unwrap_or("#6366f1") });
            let result = runtime
                .core
                .add_note_group(data)
                .await
        ...
        NoteCommands::UpdateGroup { id, name, color } => {
            let mut updates = serde_json::Map::new();
            if let Some(n) = name {
                updates.insert("name".to_string(), serde_json::Value::String(n.clone()));
            }
            if let Some(c) = color {
                updates.insert("color".to_string(), serde_json::Value::String(c.clone()));
            }
            let result = runtime
                .core
