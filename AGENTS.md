...
- **文件内容读取只有一个入口**：`safety::read_text_file_in`（只允许部署日志目录、canonicalize 后前缀校验）——**不要**把已有的 `commands::cicd::read_log_file`（无路径校验）包成工具。路径元信息检索是刻意开的例外（`assistant/paths.rs` 的 `find_local_path` / `inspect_local_path` / `detect_local_project`，后者复用向导同一套 `scan_project_impl`）：只返回路径/类型/大小/构建标志，三条硬约束必须保持——遍历仅限搜索根（主目录 + `git_scan_directories` + 应用数据目录，深度/访问数/结果数三重上限、不跟随软链）、`.ssh`/钥匙串/`.aws`/`.supertool` 等凭据位置既不可枚举也不可 stat、任何情况不返回文件内容
