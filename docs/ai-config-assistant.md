...
2. **没有文件、shell、SQL、网络工具**：唯一的本地读取入口是 `read_text_file_in`，
...
3. **不能让助手知道密码类信息**：
   - 返回值统一过 `redact_secrets`（覆盖 `password/passwd/pwd/passphrase/apiKey/secret/token/privateKey/presharedKey/sshKeyPath/keyPath/keyFile/credential/authorization/sessionKey/certPassword/totpSecret`，
