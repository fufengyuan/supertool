...
- **@ts-nocheck 状态**：仍有 38 个文件保留（`src/utils/tauri-api.ts` 及 server/vpn/projects/nginx/mfa 等模块部分文件），属历史技术债，逐个清理中。改 tauri-api 时**必须同步实现与接口声明**（TauriAPI interface），否则其他文件 vue-tsc 会报错。
