...
- **DevTools SPA 代码块提取（CodeMirror）**（2026-08）：支付宝等文档站的代码块用 **CodeMirror** 渲染（页面加载 `CodeMirror.js`），不是标准 `<pre><code>`：① 结构是 `.CodeMirror > .CodeMirror-code > pre.CodeMirror-line`（每行一个 pre），必须提取时按行拼接转成标准 `<pre><code>`（行号在 `.CodeMirror-gutter .CodeMirror-linenumber`，选择 `.CodeMirror-code .CodeMirror-line` 可排除）；② 代码块可能**默认折叠**（`ne-codeblock-collapsed-button` 按钮，CodeMirror 懒渲染折叠时 DOM 无代码）——提取前要 `click()` 展开，采样判定（SAMPLE_JS）须把 `.CodeMirror` 的 innerText 计入，否则折叠代码块文本不触发渲染完成判定；③ 判定方式：从页面 HTML 找 `CodeMirror.js`/chunk 里 `className:"ne-code"`、`collapsed` 状态确认；验证用 happy-dom 构造 `.CodeMirror` 结构跑 EXTRACT_JS。
- **DevTools SPA ne-doc 标签适配**（2026-08）：支付宝等阿里系文档站正文用飞书/Lark 文档引擎（**ne-doc**）渲染，自定义标签 `ne-p`/`ne-text`/`ne-strong`/`ne-em`/`ne-uli`/`ne-table`/`ne-card`/`ne-viewer-b-filler`——turndown 不识别导致段落不换行、表格留原始 HTML、代码块变纯文本。EXTRACT_JS 必须规范化：ne-text→文本（ne-bold=true→strong）、ne-p→p、ne-uli→li（删 ne-uli-i 符号列、ne-uli-c→span、相邻 li 包 ul）、ne-table→标准 table+**首行 td→th**（GFM 表格要求表头）、ne-card→img（只留 src/alt）、ne-viewer-b-filler 删除。用 `renameTag`（移动子节点）而非 innerHTML 复制（自定义标签序列化会丢内容）。**执行顺序关键**：normalizeNeTags → convertCodeBlocks → preserveNewlines（代码内 \n 必须在转 pre 前保留，否则被 br 破坏）。验证用 happy-dom 构造 ne-* 结构跑 EXTRACT_JS + turndown 链路。
- **DevTools SPA 懒加载触发**（2026-08）：支付宝等文档页用 **IntersectionObserver 懒加载**（代码块等元素进入视口才渲染），隐藏 WebView 不滚动则代码块永不在视口 → 抓不到。SAMPLE_JS 必须**交替滚动**（`window.__spaScrollToggle` 到底/回顶，700ms 轮询间隔天然分帧）触发 IO；EXTRACT_JS 提取前再滚到底回顶补漏。滚动会使文本持续增长 → 稳定判定（连续两次采样相同）自然重置，最终全量加载后稳定。
- **DevTools ne-doc 表格单行化**（2026-08）：ne-doc 表格 `ne-td > div.ne-td-content > ne-p` 嵌套块级，turndown 对 td 内块级输出换行 → GFM 表格单元格跨行断裂渲染不出。EXTRACT_JS 的 ne-table 段必须：td/th 内 `div/p` 降级为 `span`（renameTag）、`br` 删除、文本空白折叠为单空格（foldText 递归），preserveNewlines 跳过 `TABLE`。`\_`（下划线转义）渲染正常属 turndown 默认行为，非 bug。
- **数据库管理 ES 模块**（2026-08）：连接类型新增 `elasticsearch`（复用 `db_connections` 表，Basic Auth 可选、默认端口 9200，密码复用 `try_decrypt_password` 密文/明文兼容）。后端走 `tauri/src/commands/es.rs`（reqwest REST 直连，`EsClient` 存入 CONNECTION_POOL 的 `DbConnection::Elasticsearch` variant，`db_connect`/`db_test` 已加 `elasticsearch` 分支）；前端 `EsManager.vue` 以 `esManager` tab 打开（`TabType`/`openEsManagerTab`），连接树展开时 ES 跳过 SQL 数据库加载。易错点：① ES `_stats` 响应 `indices[name]` 下**直接是 `primaries`/`total`**（无 `_all`），文档数/存储取 `stats.indices[name].total`；② 字段化过滤 DSL 由前端 `buildQuery` 生成 bool `must`/`must_not`/`filter`：keyword→`term`、text→`match`、数字/日期→`range`、布尔→`term true/false`，全文关键词走 `query_string`；③ `_cat/indices` 的 `docs.count`/`store.size` 是字符串需 `Number()` 转换；④ 文档删除用 `@tauri-apps/plugin-dialog` 的 `confirm()`（勿用原生）；⑤ 删除多索引时其中一个不存在会整体 404（ES 语义，错误信息透传即可）。
- **CICD 复制配置工具路径覆盖**（2026-08）：复制配置后 `runToolDetection()` 曾**无条件**用 `detectToolPaths` 覆盖 `config.javaHome/mavenHome` 等 → 把 sdkman java 覆盖成 `/usr`，maven 部署时 mvn 用 `/usr/bin/java`（macOS GraalVM stub）卡死在启动检查、零输出、部署停在 build 阶段。已改为**空才回填**（与 SDK 段/loadConfig 一致）。约定：`runToolDetection`/工具路径填充一律空才回填，禁止无条件覆盖用户已配置路径。易错点：maven 配置 `javaHome=/usr` 必卡（`/usr/bin/java -version` 在后台 spawn 下会弹安装框/卡死）。
- **CICD 部署命令超时**（2026-08）：`cicd_deploy.rs` 的 git/npm/mvn 命令原均 `.output()` 无超时，遇 `.git/index.lock` 死等、SSH 认证、网络阻塞会**无限挂起**（deploy_logs 永久 running progress 0）。已加 `with_timeout` 阶段超时（git 300s/deps 600s/build 900s/ssh 600s/restart 120s）+ `kill_on_drop(true)`（超时杀子进程释放锁）+ `clean_git_index_lock`（只删 mtime>60s 残留锁）。约定：部署引擎新命令必须带超时；git_clone 模式所有配置共用 `cicd_workspace_dir()/repo_name` 工作目录，多配置会共享（勿按配置隔离假设）。注意 `kill_on_drop` 只杀直接子进程，sh 包装命令的孙进程可能残留（可接受）。
- **plugin-dialog confirm 必须 await**（2026-08）：`@tauri-apps/plugin-dialog` 的 `confirm` 返回 `Promise<boolean>`，漏 `await` 时 `proceed` 恒为 truthy → 确认框永不弹出、破坏性操作直接执行（DeployPanel 回滚审核分支曾漏 await）。规则：引入 plugin-dialog 后**所有** `confirm(` 调用都要 `await`（含 if 分支），改前 grep 全文件确认无遗漏。
- **CICD 依赖更新检测**（2026-08）：git_clone 模式 `install_dependencies` 原「node_modules 非空即跳过」导致 pull 新依赖后构建用旧依赖报错。已改为 `deps_need_update`（core/cicd_deploy.rs）：比较顶层 lock（`package-lock.json`/`pnpm-lock.yaml`/`yarn.lock`）与 node_modules 内安装副本（`.package-lock.json`/`.pnpm/lock.yaml`/`.yarn-integrity`）的 **mtime**，lock 更新才重装，install 后自然跳过。易错点：① **勿用内容 hash 比较**——npm 顶层 lock 含 root `""` 条目而安装副本不含，内容必然不同会恒重装；② 安装副本不存在（yarn berry 无 `.yarn-integrity`）时退回与 `node_modules` 目录 mtime 比较；③ 仅 git_clone 模式走 `install_dependencies`，local 模式不装依赖。
